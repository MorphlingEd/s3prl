...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, 
                ...
                padding_mask=None, 
                layer=None,
                attn_selected=None # New parameter added for outputing attention maps for selected layers
                ):
        x, layer_results = self.extract_features(x, padding_mask, layer, attn_selected=attn_selected)
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                x, (z, lr) = layer(
                    x, 
                    self_attn_padding_mask=padding_mask, 
                    need_weights= (i in attn_selected) if attn_selected is not None else False # False
                )
                print(f"i = {i}, min_layer = {min_layer}")
                if i >= min_layer:
